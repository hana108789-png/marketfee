...
(function (root) {
  ...
  // How the two headline numbers are reached, written out with the visitor's own figures.
  // Every market goes through SF.calc, so these two lines hold for all of them.
  function formula(L, v, r, fmt) {
    var esc = SF.esc, n = SF.n, t = L.t;
    var lbl = function (k) { return esc(L.field({ l: k })) + " "; };
    var term = function (sign, k, x) { return n(x) ? " " + sign + " " + lbl(k) + fmt.format(n(x)) : ""; };
    return "<div class=\"calc-f\"><span>" + esc(t.formula) + "</span>" +
      "<p>" + lbl("price") + fmt.format(n(v.price)) + term("+", "shipping", v.shipping) +
      " − " + esc(t.feesTotal) + " " + fmt.format(r.total) +
      " = <b>" + esc(t.payout) + " " + fmt.format(r.payout) + "</b></p>" +
      "<p>" + esc(t.payout) + " " + fmt.format(r.payout) + term("−", "cost", v.cost) + term("−", "shipCost", v.shipCost) +
      " = <b>" + esc(t.profit) + " " + fmt.format(r.profit) + "</b></p></div>";
  }
  ...
  SF.I18N = {
    ...
    fr: { formula: "Calcul", scenarioH: "Bénéfice selon le prix", scenarioLead: "Le même article à cinq niveaux de prix. Coût d’achat et expédition restent identiques.", guidesH: "Guides des frais", guideLink: "Lire le guide complet des frais", guideCta: "Essayer dans le calculateur", caseH: "Cas", advanced: "Paramètres avancés", sysfeeShort: "Frais système", marketplace: "Marketplace", compareH: "Comparer les marketplaces", sharedInputs: "Votre produit", bestFirst: "Trié par bénéfice net", langName: 'Français', languages: 'Langues', price: 'Prix de vente', shipping: 'Frais de port facturés à l’acheteur', cost: 'Coût d’achat', shipCost: 'Vos frais d’expédition', category: 'Catégorie', commission: 'Commission %', orders: 'Commandes par mois', vatStatus: 'Statut TVA', vatDeduct: 'Assujetti à la TVA – TVA sur les frais récupérable', vatCost: 'Non assujetti (franchise en base) – TVA sur les frais = coût', otherPct: 'Autres coûts % (pub, etc.)', otherCost: 'Autres coûts', vatOnFees: 'TVA sur les frais', targetProfit: 'Bénéfice cible (optionnel)', results: 'Résultats', feesTotal: 'Total des frais', payout: 'Montant reversé', profit: 'Bénéfice net', margin: 'Marge', roi: 'ROI', breakEven: 'Prix d’équilibre', requiredPrice: 'Prix pour le bénéfice cible', feeBreakdown: 'Détail des frais', editableNote: 'Les taux sont préremplis d’après la grille officielle et entièrement modifiables.', disclaimer: 'Estimation uniquement. Vérifiez les taux en vigueur dans votre espace vendeur.', updated: 'Mise à jour', sources: 'Sources', otherCalcs: 'Autres calculateurs', reset: 'Réinitialiser', feeTableH: 'Grille des frais', faqH: 'Questions fréquentes', home: 'Tous les calculateurs', ratesH: 'Taux',
